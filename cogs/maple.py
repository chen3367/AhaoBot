    ...
    async def calculate_equivalent(self, context: Context, ign: str, item: str, value: int, defense_p: int = 300) -> None:
        try:
            ...
            if not search_result:
                ...
                embed.add_field(name="攻擊", value=result['ATTACK'])
                embed.add_field(name="%攻擊", value=result['ATTACK_P'])
                embed.add_field(name="%總傷", value=result['DMG_P'])
                embed.add_field(name="%BOSS", value=result['BOSS_P'])
                embed.add_field(name="%爆傷", value=result['STRIKE_P'])
                embed.add_field(name="%無視", value=result['IGNORE_P'])
                # embed.add_field(name="最終傷害%", value=result['FINALDMG_P'])
                embed.add_field(name="力量", value=result['STR_CLEAR'])
                embed.add_field(name="%力量", value=result['STR_P'])
                embed.add_field(name="不吃%力", value=result['STR_UNIQUE'])
                embed.add_field(name="敏捷", value=result['DEX_CLEAR'])
                embed.add_field(name="%敏捷", value=result['DEX_P'])
                embed.add_field(name="不吃%敏", value=result['DEX_UNIQUE'])
                embed.add_field(name="智力", value=result['INT_CLEAR'])
                embed.add_field(name="%智力", value=result['INT_P'])
                embed.add_field(name="不吃%智", value=result['INT_UNIQUE'])
                embed.add_field(name="幸運", value=result['LUK_CLEAR'])
                embed.add_field(name="%幸運", value=result['LUK_P'])
                embed.add_field(name="不吃%幸", value=result['LUK_UNIQUE'])
                embed.add_field(name="%全屬", value=result['ALL_P'])
                embed.set_thumbnail(url="https://maplestory.io/api/TWMS/256/npc/9000030/icon")
        except Exception as e:
            embed = discord.Embed(
                title="錯誤", description=e, color=0xE02B2B
